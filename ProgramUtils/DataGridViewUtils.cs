...
namespace Utils
{
    public class DataGridViewUtils
    {
        ...
        private static Array GridToArrayInner<T>(DataGridView dgv, Action<DataGridViewRow, T> processFunc)
           where T : new()
        {
            Array result = (Array)new T[dgv.RowCount];
            ...
            for (int r = 0; r < dgv.RowCount; r++)
            {
                ...
                {
                    T obj = new T();
                    processFunc(dgv.Rows[r], obj);

                    result.SetValue(obj, r);
                }
                catch (Exception except)
                {
                    throw new ApplicationException("Вы допустили ошибку при вводе данных");
                }
            }
        ...
        // Создание списка объектов с определенныйми свойствами

        public static List<T> GridToObjectsList<T>(DataGridView dgv, Action<DataGridViewRow, T> processFunc)
            where T : new()
        {
            return new List<T>((T[])GridToArrayInner<T>(dgv, processFunc));
        }
